...
class AdvancedSourceFilterTest {
    ...
    fun setup() {
        ...
        source1080pHEVC = createTestSource(
            id = "1080p_hevc",
        ...
        source720pH264 = createTestSource(
            id = "720p_h264",
        ...
        sourceDebridCached = createTestSource(
            id = "debrid_cached",
            resolution = VideoResolution.RESOLUTION_1080P,
            hdr10 = false,
            codec = VideoCodec.H264,
        ...
        sourceTorrentHighSeed = createTestSource(
        ...
        sourceTorrentLowSeed = createTestSource(
            id = "torrent_low_seed",
            resolution = VideoResolution.RESOLUTION_1080P,
            hdr10 = false,
            codec = VideoCodec.H264,
        ...
        sourceRemuxLarge = createTestSource(
            id = "remux_large",
            resolution = VideoResolution.RESOLUTION_1080P,
            hdr10Plus = true,
            codec = VideoCodec.H264,
        ...
        sourceWebDLSmall = createTestSource(
            id = "webdl_small",
            resolution = VideoResolution.RESOLUTION_720P,
            hdr10 = false,
            codec = VideoCodec.H264,
    ...
    fun `AdvancedSourceFilter isEmpty works correctly`() {
        ...
        val nonEmptyFilter = AdvancedSourceFilter(
    ...
    fun `AdvancedSourceFilter getActiveFilterCount works correctly`() {
        ...
        val multiFilter = AdvancedSourceFilter(
            qualityFilters = QualityFilters(minResolution = VideoResolution.RESOLUTION_1080P),
    ...
    fun `QualityFilters work correctly`() {
        val qualityFilter = AdvancedSourceFilter(
        ...
        val filteredSources = testSources.filter { source ->
    ...
    fun `SourceTypeFilters work correctly`() {
        val cachedOnlyFilter = AdvancedSourceFilter(
            sourceTypeFilters = SourceTypeFilters(cachedOnly = true)
        )

        val p2pOnlyFilter = AdvancedSourceFilter(
            sourceTypeFilters = SourceTypeFilters(p2pOnly = true)
        )

        val cachedSources = testSources.filter { it.availability.cached }
        val p2pSources = testSources.filter { 
    ...
    fun `HealthFilters work correctly`() {
        val healthFilter = AdvancedSourceFilter(
        ...
        val healthySources = testSources.filter { source ->
    ...
    fun `FileSizeFilters work correctly`() {
        val sizeFilter = AdvancedSourceFilter(
        ...
        val sizedSources = testSources.filter { source ->
    ...
    fun `CodecFilters work correctly`() {
        val hevcOnlyFilter = AdvancedSourceFilter(
            codecFilters = CodecFilters(
                allowedCodecs = setOf(VideoCodec.HEVC)
            )
        )

        val hevcSources = testSources.filter { source ->
    ...
    fun `AudioFilters work correctly`() {
        val losslessAudioFilter = AdvancedSourceFilter(
            audioFilters = AudioFilters(
                allowedFormats = setOf(AudioFormat.TRUEHD, AudioFormat.DTS_HD_MA, AudioFormat.FLAC)
            )
        )

        val losslessAudioSources = testSources.filter { source ->
    ...
    fun `ReleaseTypeFilters work correctly`() {
        val remuxOnlyFilter = AdvancedSourceFilter(
            releaseTypeFilters = ReleaseTypeFilters(
                allowedTypes = setOf(ReleaseType.BLURAY_REMUX)
            )
        )

        val remuxSources = testSources.filter { source ->
    ...
    fun `ProviderFilters work correctly`() {
        val debridOnlyFilter = AdvancedSourceFilter(
            providerFilters = ProviderFilters(
                allowedProviders = setOf("provider_debrid_cached")
            )
        )

        val debridSources = testSources.filter { source ->
    ...
    fun `complex filter combinations work correctly`() {
        ...
        val filteredSources = testSources.filter { source ->
            ...
            // Audio check
            source.audio.format in setOf(
                AudioFormat.TRUEHD, AudioFormat.DTS_HD_MA, 
                AudioFormat.EAC3, AudioFormat.AC3
            ) &&
            // Release type check
            source.release.type in setOf(
                ReleaseType.BLURAY, ReleaseType.BLURAY_REMUX, 
                ReleaseType.WEB_DL
            )
        }
    ...
    fun `filter with AND combination works correctly`() {
        val andFilter = AdvancedSourceFilter(
            qualityFilters = QualityFilters(minResolution = VideoResolution.RESOLUTION_1080P),
        ...
        val andResults = testSources.filter { source ->
            source.quality.resolution.ordinal >= VideoResolution.RESOLUTION_1080P.ordinal &&
        ...
        val correctAndResults = testSources.filter { source ->
            source.quality.resolution.ordinal >= VideoResolution.RESOLUTION_1080P.ordinal &&
            (source.health.seeders ?: 0) >= 200
        }
    ...
    fun `advanced filter handles edge cases gracefully`() {
        ...
        val impossibleFilter = AdvancedSourceFilter(
            qualityFilters = QualityFilters(
