...
class SourceComparatorsTest {
    ...
    fun setup() {
        ...
        sourceHighSeeders = createTestSource(
            id = "high_seeders",
            resolution = VideoResolution.RESOLUTION_1080P,
            seeders = 1000,
            codec = VideoCodec.H264,
        ...
        sourceLowSeeders = createTestSource(
            id = "low_seeders",
            resolution = VideoResolution.RESOLUTION_1080P,
            seeders = 5,
            codec = VideoCodec.H264,
        ...
        sourceTorrent = createTestSource(
            id = "torrent_source",
            resolution = VideoResolution.RESOLUTION_1080P,
            seeders = 100,
            codec = VideoCodec.H264,
    ...
    fun `debridFirstComparator - prioritizes debrid sources`() {
        ...
        for (i in 0 until sorted.size - 1) {
            ...
            val currentPriority = when (currentType) {
            ...
            val nextPriority = when (nextType) {
                SourceProviderInfo.ProviderType.DEBRID -> 0
                SourceProviderInfo.ProviderType.DIRECT_STREAM -> 1
                SourceProviderInfo.ProviderType.TORRENT -> 2
                else -> 3
    ...
    fun `createCompositeComparator - combines multiple comparators correctly`() {
        val compositeComparator = SourceComparators.createCompositeComparator(
    ...
    fun `createWeightedComparator - balances multiple criteria`() {
        val weightedComparator = SourceComparators.createWeightedComparator(
            qualityWeight = 1.0f,
            healthWeight = 0.5f,
            sizeWeight = 0.2f,
            providerWeight = 0.3f
    ...
    fun `chain - combines multiple comparators in order`() {
        val chainedComparator = SourceComparators.chain(
