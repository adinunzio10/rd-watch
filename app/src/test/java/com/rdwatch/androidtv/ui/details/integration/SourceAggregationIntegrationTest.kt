...
class SourceAggregationIntegrationTest {
    ...
    @Test
    fun `source aggregation pipeline processes all sources correctly`() = runTest {
    ...
    @Test
    fun `quality scoring works correctly across different source types`() = runTest {
    ...
    @Test
    fun `filtering system works correctly with complex criteria`() = runTest {
    ...
    @Test
    fun `sorting algorithms produce consistent results`() = runTest {
    ...
    @Test
    fun `health monitoring integration works correctly`() = runTest {
        ...
        val excellentHealthData = healthMonitor.calculateHealthScore(
            excellentSource.health, 
            excellentSource.provider
        )
        val poorHealthData = healthMonitor.calculateHealthScore(
            poorSource.health, 
            poorSource.provider
        )
    ...
    @Test
    fun `season pack detection works in integration`() = runTest {
        val seasonPackSource = testSources.find { it.id == "season_pack" }!!
        val seasonPackInfo = seasonPackDetector.analyzeSeasonPack(
    ...
    @Test
    fun `full pipeline with filters and sorting works correctly`() = runTest {
    ...
    @Test
    fun `provider reliability affects final ranking`() = runTest {
    ...
    @Test
    fun `large source list performance`() = runTest {
        // Create a large number of sources
        val largeSources = (1..1000).map { index ->
            createTestSource(
                id = "source_$index",
        ...
        val filtered = largeSources.filter { source ->
            source.quality.resolution.ordinal >= VideoResolution.RESOLUTION_1080P.ordinal &&
            (source.health.seeders ?: 0) >= 100
        }
    ...
    @Test
    fun `error handling in aggregation pipeline`() = runTest {
        // Test with corrupted source data
        val corruptedSources = listOf(
            createTestSource(
                id = "corrupted",
