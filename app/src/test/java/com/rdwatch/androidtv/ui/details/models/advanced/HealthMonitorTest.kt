...
class HealthMonitorTest {
    ...
    fun setup() {
        ...
        testProvider = SourceProviderInfo(
            id = "test_provider",
            name = "Test Provider",
    ...
    fun `calculateHealthScore - provider reliability affects score`() {
        val excellentProvider = testProvider.copy(
            reliability = SourceProviderInfo.ProviderReliability.EXCELLENT
        )
        val poorProvider = testProvider.copy(
    ...
    fun `calculateHealthScore - debrid provider gets bonus`() {
        val debridProvider = testProvider.copy(
            type = SourceProviderInfo.ProviderType.DEBRID
        )
        val torrentProvider = testProvider.copy(
            type = SourceProviderInfo.ProviderType.TORRENT
        )
    ...
    @Test
    fun `cacheHealthData - stores and retrieves correctly`() = runTest {
    ...
    @Test
    fun `cacheHealthData - updates state flow`() = runTest {
    ...
    @Test
    fun `health monitoring - maintains history for predictions`() = runTest {
    ...
    fun `ratio calculation handles edge cases`() {
        val noLeechersHealth = HealthInfo(
            seeders = 100,
            leechers = 0,
            availability = 1.0f,
            lastChecked = Date()
    ...
    fun `provider capabilities affect authority score`() {
        ...
        val advancedProvider = testProvider.copy(
