...
fun <T> Flow<T>.throttleFirst(windowDurationMillis: Long): Flow<T> = debounce(windowDurationMillis)

fun <T> Flow<T>.throttleLatest(timeoutMs: Long): Flow<T> = debounce(timeoutMs)

fun <T> Flow<T>.switchToIo(dispatcherProvider: DispatcherProvider): Flow<T> = flowOn(dispatcherProvider.io)

fun <T> Flow<T>.switchToMain(dispatcherProvider: DispatcherProvider): Flow<T> = flowOn(dispatcherProvider.main)

fun <T> Flow<T>.switchToDefault(dispatcherProvider: DispatcherProvider): Flow<T> = flowOn(dispatcherProvider.default)

fun <T> Flow<T>.switchTo(dispatcher: CoroutineDispatcher): Flow<T> = flowOn(dispatcher)

fun <T> Flow<T>.debounceSearch(timeoutMillis: Long = 300): Flow<T> = debounce(timeoutMillis)

fun <T> Flow<T>.throttleClicks(windowDurationMillis: Long = 300): Flow<T> = throttleFirst(windowDurationMillis)

fun <T> Flow<T>.throttleLatestWithTimeout(timeoutMs: Long = 1000): Flow<T> = throttleLatest(timeoutMs)
