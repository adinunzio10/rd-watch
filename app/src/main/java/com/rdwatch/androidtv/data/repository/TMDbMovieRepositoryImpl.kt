        ...
        override fun getMovieDetails(
            ...
            networkBoundResource(
                loadFromDb = {
                    ...
                    val dbFlow =
                        tmdbMovieDao.getMovieById(movieId).map {
                            val result = it?.toMovieResponse()
                            android.util.Log.d(
                                "TMDbMovieRepo",
                                "DB returned: ${if (result != null) "cached movie ${result.title}" else "null (no cached data)"}",
                            )
                            result
                        }
                    dbFlow
            ...
            networkBoundResource(
                ...
                saveCallResult = { discoveryResponse ->
                    ...
                    tmdbSearchDao.insertSearchResult(
                        (discoveryResponse as TMDbSearchResponse).toEntity(discoveryId, "discover", page, "movie"),
                    )
                },
            )
                ...
                if (body != null) {
                    ...
                    when (body) {
                        ...
                        is TMDbRecommendationsResponse -> {
                            android.util.Log.d(
                                "TMDbMovieRepo",
                                "TMDbRecommendationsResponse with ${body.results.size} results, page ${body.page}/${body.totalPages}",
                            )
                        }
                    }
                }
