...
interface WatchProgressDao {
    ...
    suspend fun getWatchProgress(userId: Long, contentId: String): WatchProgressEntity?
    
    @Query("SELECT * FROM watch_progress WHERE user_id = :userId AND content_id = :contentId")
    fun getWatchProgressFlow(userId: Long, contentId: String): Flow<WatchProgressEntity?>
    ...
    fun getWatchProgressWithMinPercentage(userId: Long, minPercentage: Float): Flow<List<WatchProgressEntity>>
    
    @Query("SELECT DISTINCT content_id FROM watch_progress WHERE user_id = :userId ORDER BY updated_at DESC LIMIT :limit")
    suspend fun getRecentlyWatchedContentIds(userId: Long, limit: Int = 50): List<String>
    ...
    suspend fun markAsCompleted(userId: Long, contentId: String)
    ...
    suspend fun deleteWatchProgress(userId: Long, contentId: String)
