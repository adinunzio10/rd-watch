...
data class TMDbMultiSearchResultContentDetail(
    ...
    fun withDownloadStatus(
        downloaded: Boolean,
        downloading: Boolean = false,
    ): TMDbMultiSearchResultContentDetail {
        return copy(isDownloaded = downloaded, isDownloading = downloading)
    }
...
data class TMDbEpisodeContentDetail(
    ...
    private val isDownloaded: Boolean = false,
    private val isDownloading: Boolean = false,
    private val episodeExternalIds: TMDbExternalIdsResponse? = null,
) : ContentDetail {
    ...
    override val metadata: ContentMetadata =
        ContentMetadata(
            ...
            customMetadata =
                buildMap {
                    put("tmdb_id", tmdbEpisode.id.toString())
                    put("tv_show_name", tmdbTV?.name ?: "")
                    put("tv_show_id", tmdbTV?.id?.toString() ?: "")
                    put("season_number", tmdbEpisode.seasonNumber.toString())
                    put("episode_number", tmdbEpisode.episodeNumber.toString())
                    put("air_date", tmdbEpisode.airDate ?: "")
                    put("vote_count", tmdbEpisode.voteCount.toString())
                    put("production_code", tmdbEpisode.productionCode)
                    put("guest_stars", tmdbEpisode.guestStars.joinToString(", ") { it.name })
                    put("crew", tmdbEpisode.crew.joinToString(", ") { "${it.name} (${it.job})" })

                    // External IDs for enhanced source scraping (Torrentio compatibility)
    ...
    /**
     * Get all episode external IDs
     */
    fun getEpisodeExternalIds(): TMDbExternalIdsResponse? = episodeExternalIds

    /**
     * Check if episode has IMDb ID available for source scraping
     */
    fun hasEpisodeImdbId(): Boolean = !episodeExternalIds?.imdbId.isNullOrEmpty()
