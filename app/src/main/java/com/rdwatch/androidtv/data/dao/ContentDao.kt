...
interface ContentDao {
    ...
    fun searchContentBySource(query: String, source: ContentSource): Flow<List<ContentEntity>>
    ...
    suspend fun updateFavoriteStatus(contentId: Long, isFavorite: Boolean)
    
    @Query("UPDATE content SET isWatched = :isWatched WHERE id = :contentId")
    suspend fun updateWatchedStatus(contentId: Long, isWatched: Boolean)
    
    @Query("UPDATE content SET lastPlayedDate = :date, playCount = playCount + 1 WHERE id = :contentId")
    suspend fun updatePlayedInfo(contentId: Long, date: Long = System.currentTimeMillis())
}