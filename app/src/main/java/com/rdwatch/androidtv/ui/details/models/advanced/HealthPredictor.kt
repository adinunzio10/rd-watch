...
class HealthPredictor {
    ...
    fun estimateDownloadTime(
        ...
        val combinedEstimation =
            combineTimeEstimations(
                p2pEstimation,
                networkEstimation,
        ...
        val confidence =
            calculateTimeEstimationConfidence(
                sourceMetadata,
                healthData,
                listOfNotNull(p2pEstimation, networkEstimation, historicalEstimation),
            )
    ...
    private fun calculateBaseReliability(
        sourceMetadata: SourceMetadata,
        healthData: HealthData,
    ): Int {
        ...
        reliability +=
            when (sourceMetadata.provider.type) {
        ...
        reliability +=
            when (sourceMetadata.release.type) {
    ...
    private fun estimateP2PDownloadTime(
        fileSize: Long,
        healthData: HealthData,
    ): TimeEstimation? {
        ...
        return TimeEstimation(
            ...
            factors =
                listOf(
                    "Current download speed: ${downloadSpeed / 1_000_000} MB/s",
    ...
    private fun estimateNetworkDownloadTime(
        fileSize: Long,
        userSpeedMbps: Int?,
    ): TimeEstimation? {
    ...
    private fun estimateFromHistoricalData(
        providerId: String,
        fileSize: Long,
    ): TimeEstimation? {
        val history = providerHistory[providerId] ?: return null
        val similarDownloads =
            history.downloads.filter {
    ...
    private fun assessQualityRisks(
        quality: QualityInfo,
        codec: CodecInfo,
    ...
    private fun assessUserSpecificRisks(
        sourceMetadata: SourceMetadata,
