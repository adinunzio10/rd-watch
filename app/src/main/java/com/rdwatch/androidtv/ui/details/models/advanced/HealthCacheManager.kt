...
class HealthCacheManager(private val context: Context) {
    ...
    suspend fun optimizeCache() {
        ...
        val expiredMemoryKeys =
            memoryCache.entries.filter { (_, entry) ->
    ...
    private fun storeInMemoryCache(
        sourceId: String,
        entry: CachedHealthEntry,
    ) {
    ...
    private suspend fun storeToDiskCache(
        sourceId: String,
        entry: CachedHealthEntry,
    ) {
        try {
    ...
    private suspend fun storeToPersistentCache(
        sourceId: String,
        entry: CachedHealthEntry,
    ) {
        try {
