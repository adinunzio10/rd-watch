...
object FilterPresets {
     ...
     */
    val BEST_QUALITY = AdvancedSourceFilter(
     ...
     */
    val HDR_ONLY = AdvancedSourceFilter(
        qualityFilters = QualityFilters(
            requireHDR = true,
            minResolution = VideoResolution.RESOLUTION_1080P
        ),
     ...
     */
    val UHD_4K = AdvancedSourceFilter(
        qualityFilters = QualityFilters(
            require4KOnly = true,
     ...
     */
    val CACHED_ONLY = AdvancedSourceFilter(
     ...
     */
    val SMALL_FILES = AdvancedSourceFilter(
        fileSizeFilters = FileSizeFilters(
            maxSizeGB = 5.0,
     ...
     */
    val HIGH_SEEDERS = AdvancedSourceFilter(
        sourceTypeFilters = SourceTypeFilters(
            p2pOnly = true
        ),
     ...
     */
    val TV_OPTIMIZED = AdvancedSourceFilter(
        qualityFilters = QualityFilters(
            minResolution = VideoResolution.RESOLUTION_1080P,
     ...
     */
    val AUDIOPHILE = AdvancedSourceFilter(
        audioFilters = AudioFilters(
            allowedFormats = setOf(
                AudioFormat.TRUEHD,
                AudioFormat.DTS_HD_MA,
     ...
     */
    val QUICK_DOWNLOAD = AdvancedSourceFilter(
        fileSizeFilters = FileSizeFilters(
            maxSizeGB = 3.0
        ),
        healthFilters = HealthFilters(
     ...
     */
    val ARCHIVE_QUALITY = AdvancedSourceFilter(
        qualityFilters = QualityFilters(
            minResolution = VideoResolution.RESOLUTION_1080P
        ),
        releaseTypeFilters = ReleaseTypeFilters(
    ...
    fun getPresetsByCategory(): Map<PresetCategory, List<Pair<String, AdvancedSourceFilter>>> {
        return mapOf(
            PresetCategory.QUALITY to listOf(
                "Best Quality" to BEST_QUALITY,
                "HDR Only" to HDR_ONLY,
                "4K UHD" to UHD_4K,
                "Archive Quality" to ARCHIVE_QUALITY
            ),
            PresetCategory.SIZE to listOf(
                "Small Files" to SMALL_FILES,
                "Quick Download" to QUICK_DOWNLOAD
            ),
            PresetCategory.SPEED to listOf(
                "Cached Only" to CACHED_ONLY,
                "High Seeders" to HIGH_SEEDERS,
                "Quick Download" to QUICK_DOWNLOAD
            ),
            PresetCategory.DEVICE to listOf(
                "TV Optimized" to TV_OPTIMIZED
            ),
            PresetCategory.AUDIO to listOf(
                "Audiophile" to AUDIOPHILE
            )
        )
    }
