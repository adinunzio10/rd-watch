...
class SearchRateLimiter @Inject constructor() {
     ...
     */
    suspend fun acquirePermit(scraperId: String, config: RateLimitConfig = defaultConfig) {
        // Acquire global permit first
        globalSemaphore.acquire()
        
        try {
            val scraperLimit = scraperLimits.getOrPut(scraperId) {
    ...
    fun getRateLimitStatus(scraperId: String): RateLimitStatus {
        val scraperLimit = scraperLimits[scraperId] ?: return RateLimitStatus(
            scraperId = scraperId,
            isRateLimited = false,
     ...
     */
    fun updateScraperConfig(scraperId: String, config: RateLimitConfig) {
...
data class RateLimitConfig(
    ...
    companion object {
        ...
        fun conservative() = RateLimitConfig(
        ...
        fun aggressive() = RateLimitConfig(
