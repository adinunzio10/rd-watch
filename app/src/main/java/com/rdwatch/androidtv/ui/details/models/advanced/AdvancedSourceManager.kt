...
class AdvancedSourceManager(private val context: Context) {
    ...
    suspend fun batchProcessSources(sources: List<SourceMetadata>): List<ProcessedSourceData> {
        ...
        val processedSources =
            sources.chunked(10).flatMap { chunk: List<SourceMetadata> ->
                coroutineScope {
    ...
    private fun startMonitoringIfNeeded(sourceMetadata: SourceMetadata) {
        ...
        val shouldMonitor =
            sourceMetadata.quality.resolution.ordinal >= VideoResolution.RESOLUTION_1080P.ordinal ||
    ...
    private fun calculateRecommendationScore(
        ...
        userProfile?.let { profile ->
            ...
            profile.connectionSpeedMbps?.let { speed ->
                ...
                if (estimatedHours <= 2) {
                    score += 50 // Fast download
                } else if (estimatedHours <= 6) {
                    score += 25 // Reasonable download
                } else {
                    score -= 25 // Slow download
                }
            }
        }
    ...
    private fun generateRecommendationReasoning(
        ...
        // Performance reasons
        if (score >= 1500) {
            reasons.add("Excellent overall quality")
        } else if (score >= 1200) {
            reasons.add("Very good quality")
        } else if (score >= 900) {
            reasons.add("Good quality")
        } else if (score >= 600) {
            reasons.add("Fair quality")
        } else {
            reasons.add("Consider alternative sources")
        }
    ...
    private fun calculateUserCompatibility(
        ...
        userProfile.storageSpaceGB?.let { available ->
            val fileSizeGB =
                processed.sourceMetadata.file.sizeInBytes?.let {
