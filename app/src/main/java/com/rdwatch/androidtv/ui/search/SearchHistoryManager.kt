...
class SearchHistoryManager @Inject constructor(
     ...
     */
    fun getRecentSearchHistory(userId: Long, limit: Int = 20): Flow<List<String>> {
     ...
     */
    suspend fun getPopularSearchQueries(userId: Long, limit: Int = 10): List<String> {
     ...
     */
    fun searchInHistory(userId: Long, query: String): Flow<List<SearchHistoryItem>> {
     ...
     */
    suspend fun deleteSearchQuery(userId: Long, query: String) {
     ...
     */
    suspend fun cleanupOldHistory(userId: Long, keepCount: Int = 1000) {
    ...
    suspend fun getTrendingQueries(
        ...
        val recentHistory = searchHistoryDao.getSearchHistoryByDateRange(
            userId, cutoffDate, Date()
        )
    ...
    private fun getContextualSuggestions(context: SearchContext): List<SmartSuggestion> {
        ...
        when (month) {
            11, 0, 1 -> suggestions.add(SmartSuggestion(
                query = "winter movies",
                type = SuggestionType.CONTEXTUAL,
                confidence = 0.5,
                reason = "Winter season"
            ))
            2, 3, 4 -> suggestions.add(SmartSuggestion(
                query = "spring shows",
                type = SuggestionType.CONTEXTUAL,
                confidence = 0.5,
