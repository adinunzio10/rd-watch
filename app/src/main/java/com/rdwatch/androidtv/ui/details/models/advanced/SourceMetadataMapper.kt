...
object SourceMetadataMapper {
    ...
    fun fromStremioStream(
        ...
        return SourceMetadata(
            ...
            provider = SourceProviderInfo(
                ...
                logoUrl = null,
                type = if (stream.isTorrent()) SourceProviderInfo.ProviderType.TORRENT 
                       else SourceProviderInfo.ProviderType.DIRECT_STREAM,
                reliability = SourceProviderInfo.ProviderReliability.GOOD,
                capabilities = setOf("stream")
            ),
            quality = QualityInfo(
                resolution = torrentInfo?.quality?.let { VideoResolution.fromString(it) } 
    ...
    fun fromRealDebridTorrent(
        ...
        return SourceMetadata(
            ...
            quality = QualityInfo(
                resolution = scraperTorrentInfo.quality?.let { VideoResolution.fromString(it) } 
                           ?: VideoResolution.UNKNOWN,
