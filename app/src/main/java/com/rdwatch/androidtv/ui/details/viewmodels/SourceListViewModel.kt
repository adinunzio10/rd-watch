...
/**
 * Service for managing source list state, filtering, sorting, and data operations
 * Integrates with the Backend Agent's advanced sorting algorithms
 * Note: This is a service class, not a ViewModel, to allow injection into other ViewModels
 */
@Singleton
class SourceListViewModel
    ...
    constructor(
        ...
        fun loadSources(
            ...
            serviceScope.launch {
                ...
                try {
                    ...
                    // Create basic Movie and ContentDetail for the repository
                    val sampleMovie =
                        com.rdwatch.androidtv.Movie(
                    ...
                    // Fetch sources from repository using the proper interface
                    ...
                    selectionManager.updateSourcesForAndroidTV(sourcesList)

                    // Track successful load event
                    trackEvent(
                        SourceSelectionEvents.SourceLoadEvent(
                            sourceId = movieId,
                            provider = "aggregated",
                            success = true,
                    ...
                    // Track error event
                    trackEvent(
                        SourceSelectionEvents.SourceErrorEvent(
                            sourceId = movieId,
                            provider = "aggregated",
                            errorType = "LoadError",
        ...
        fun selectSource(source: SourceMetadata) {
            ...
            trackEvent(
                SourceSelectionEvents.SourceSelectedEvent(
                    sourceId = source.id,
        ...
        fun applyFilter(filter: SourceFilter) {
            ...
            // Track filter event
            trackEvent(
                SourceSelectionEvents.SourceFilterEvent(
        ...
        fun applySortOption(sortOption: SourceSortOption) {
            ...
            // Track sort event
            trackEvent(
                SourceSelectionEvents.SourceSortEvent(
        ...
        fun changeViewMode(viewMode: SourceSelectionState.ViewMode) {
            ...
            trackEvent(
                SourceSelectionEvents.SourceViewEvent(
        ...
        fun playSource(source: SourceMetadata) {
            // Track play event
            trackEvent(
                SourceSelectionEvents.SourcePlayEvent(
                    sourceId = source.id,
                    provider = source.provider.name,
        ...
        fun downloadSource(source: SourceMetadata) {
            ...
            trackEvent(
                SourceSelectionEvents.SourceDownloadEvent(
                    sourceId = source.id,
                    provider = source.provider.name,
        ...
        fun addToPlaylist(source: SourceMetadata) {
            ...
            trackEvent(
                SourceSelectionEvents.SourcePlaylistEvent(
                    action = "add",
                    sourceId = source.id,
                    provider = source.provider.name,
