...
class RealDebridFileBrowserRepository @Inject constructor(
    ...
    override fun getRootContent(): Flow<Result<List<FileItem>>> = flow {
        ...
        try {
            contentRepository.getAllContent().collect { result ->
                when (result) {
                    is Result.Success -> {
                        val fileItems = result.data.map { contentEntity ->
    ...
    override suspend fun getTorrentFiles(torrentId: String): Result<List<FileItem.File>> = withContext(dispatcherProvider.io) {
        // Check cache first
        ...
        safeCall {
            ...
            if (response.isSuccessful) {
                ...
                if (torrentInfo != null) {
                    val files = torrentInfo.files.map { torrentFile ->
    ...
    override suspend fun getPlaybackUrl(fileId: String, torrentId: String?): Result<String> = withContext(dispatcherProvider.io) {
    ...
    override suspend fun deleteItems(itemIds: Set<String>): Result<Unit> = withContext(dispatcherProvider.io) {
        safeCall {
    ...
    override suspend fun downloadFiles(fileIds: Set<String>): Result<Unit> = withContext(dispatcherProvider.io) {
    ...
    override fun searchContent(query: String): Flow<Result<List<FileItem>>> = 
        getRootContent().map { result ->
            when (result) {
                is Result.Success -> {
                    val filtered = result.data.filter { item ->
    ...
    override suspend fun refreshContent(): Result<Unit> = withContext(dispatcherProvider.io) {
        cacheMutex.withLock {
    ...
    override suspend fun pullToRefresh(): Result<Unit> = withContext(dispatcherProvider.io) {
        cacheMutex.withLock {
    ...
    override suspend fun getItemDetails(itemId: String): Result<FileItem> = withContext(dispatcherProvider.io) {
        safeCall {
    ...
    override suspend fun cancelOperations(): Result<Unit> = withContext(dispatcherProvider.io) {
        safeCall {
    ...
    override suspend fun getCacheInfo(): CacheInfo = withContext(dispatcherProvider.io) {
    ...
    override suspend fun clearCache(): Result<Unit> = withContext(dispatcherProvider.io) {
        safeCall {
            cacheMutex.withLock {
                invalidateAllCaches()
                Unit
