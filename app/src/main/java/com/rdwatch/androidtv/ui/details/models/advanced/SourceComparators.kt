...
object SourceComparators {
     ...
     */
    val qualityComparator = compareByDescending<SourceMetadata> { source ->
     ...
     */
    val healthComparator = compareByDescending<SourceMetadata> { source ->
     ...
     */
    val sizeComparator = compareBy<SourceMetadata> { source ->
        source.file.sizeInBytes ?: Long.MAX_VALUE
    }
    
    val sizeLargestFirstComparator = compareByDescending<SourceMetadata> { source ->
     ...
     */
    val providerReliabilityComparator = compareByDescending<SourceMetadata> { source ->
     ...
     */
    val releaseTypeComparator = compareByDescending<SourceMetadata> { source ->
     ...
     */
    val cachedFirstComparator = compareBy<SourceMetadata> { source ->
     ...
     */
    val debridFirstComparator = compareBy<SourceMetadata> { source ->
     ...
     */
    val recentComparator = compareByDescending<SourceMetadata> { source ->
     ...
     */
    val androidTVOptimizedComparator = compareBy<SourceMetadata> { source ->
        // Priority 1: Cached debrid sources (instant start)
        if (source.availability.cached && source.provider.type == SourceProviderInfo.ProviderType.DEBRID) 0
        // Priority 2: Direct streams (fast start)
        else if (source.provider.type == SourceProviderInfo.ProviderType.DIRECT_STREAM) 1
        // Priority 3: Non-cached debrid (moderate start)
        else if (source.provider.type == SourceProviderInfo.ProviderType.DEBRID) 2
        // Priority 4: High-seeded torrents (slower start)
        else if ((source.health.seeders ?: 0) > 100) 3
...
fun List<SourceMetadata>.sortBySize(largestFirst: Boolean = false): List<SourceMetadata> {
    return sortedWith(
        if (largestFirst) SourceComparators.sizeLargestFirstComparator 
        else SourceComparators.sizeComparator
    )
}
