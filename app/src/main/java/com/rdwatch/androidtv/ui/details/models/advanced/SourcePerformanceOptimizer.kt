...
class SourcePerformanceOptimizer(
    ...
    suspend fun processSourcesOptimized(
    ...
    ): ProcessedSourceResult = withContext(Dispatchers.Default) {
        ...
        try {
            ...
            val processed = if (preFiltered.size > configuration.chunkThreshold) {
    ...
    private fun preFilterSources(sources: List<SourceMetadata>): List<SourceMetadata> {
        return sources.filter { source ->
            ...
            if (source.quality.resolution == VideoResolution.RESOLUTION_240P ||
                source.quality.resolution == VideoResolution.UNKNOWN) return@filter false
            
            // Remove sources without file information if too many sources
            if (sources.size > configuration.strictFilterThreshold && 
                source.file.sizeInBytes == null) return@filter false
            
            // Remove untrusted providers if too many sources
            if (sources.size > configuration.strictFilterThreshold &&
                source.provider.reliability == SourceProviderInfo.ProviderReliability.POOR) return@filter false
    ...
    private suspend fun processSourcesInChunks(
        ...
        preferences: UserSortingPreferences,
        maxResults: Int
    ): List<SourceMetadata> = withContext(Dispatchers.Default) {
    ...
    suspend fun optimizeForAndroidTV(
        sources: List<SourceMetadata>,
        preferences: UserSortingPreferences = UserSortingPreferences()
    ): AndroidTVOptimizedResult = withContext(Dispatchers.Default) {
        val startTime = System.currentTimeMillis()
     ...
     */
    private fun getOptimizationsApplied(originalSize: Int, filteredSize: Int): List<String> {
