...
class TVDetailsViewModel
    ...
    constructor(
                                            ...
                                            TVShowDetail(
                                                ...
                                                lastEpisodeToAir =
                                                    contentDetail.getLastEpisodeToAir()?.let {
                                                        mapTMDbEpisodeToTVEpisode(
                                                            it,
                                                        )
                                                    },
                                                nextEpisodeToAir =
                                                    contentDetail.getNextEpisodeToAir()?.let {
                                                        mapTMDbEpisodeToTVEpisode(
                                                            it,
                                                        )
                                                    },
         ...
         */
        private fun loadSeasonsWithEpisodes(
            tmdbId: Int,
         ...
         */
        private fun buildSeasonsList(
            loadedSeason: TVSeason,
            existingSeasons: List<TVSeason>,
            totalSeasons: Int,
        ): List<TVSeason> {
         ...
         */
        private fun useDefaultOrExistingSeasons(
            tvShowDetail: TVShowContentDetail,
            existingSeasons: List<TVSeason>,
        ) {
            if (existingSeasons.isNotEmpty()) {
        ...
        private fun validateOnDemandLoadingPreconditions(seasonNumber: Int): OnDemandValidationResult {
            ...
            val hasValidEpisodes =
                currentSeason?.episodes?.isNotEmpty() == true &&
         ...
         */
        private fun startOnDemandSeasonLoading(
            seasonNumber: Int,
            tmdbId: Int,
        ...
        private fun handleOnDemandSeasonLoadSuccess(
            ...
            if (seasonResponse.id != 0 && (seasonResponse.episodes.isNotEmpty() || seasonResponse.episodeCount > 0)) {
                ...
                if (latestTvShow != null) {
                    ...
                    val updatedSeasons =
                        currentTvShowDetail.seasons.map { season ->
         ...
         */
        private fun handleOnDemandSeasonLoadError(
            seasonNumber: Int,
         ...
         */
        private fun updateTVShowWithSeasons(
        ...
        private fun synchronizeSeasonSelection(updatedSeasons: List<TVSeason>) {
            ...
            val updatedSelectedSeason =
                currentSelectedSeason?.let { selected ->
         ...
         */
        private fun synchronizeEpisodeSelection(
            updatedSeason: TVSeason,
        ...
        object TMDbMapper {
             ...
             */
            private fun validateEpisodeData(
                tmdbEpisode: com.rdwatch.androidtv.network.models.tmdb.TMDbEpisodeResponse,
                seasonNumber: Int,
            ) {
                if (tmdbEpisode.id == 0) {
                ...
                tmdbTVRepository.getTVCredits(tmdbId).collect { result ->
                    when (result) {
                        ...
                        is com.rdwatch.androidtv.repository.base.Result.Success -> {
                            ...
                            val castMembers =
                                creditsResponse.cast.take(20).map { tmdbCast ->
                                    CastMember(
                                        id = tmdbCast.id,
                                        name = tmdbCast.name,
                            ...
                            val crewMembers =
                                creditsResponse.crew.map { tmdbCrew ->
                                    CrewMember(
                                        id = tmdbCrew.id,
                                        name = tmdbCrew.name,
                                        job = tmdbCrew.job,
         ...
         */
        fun loadAdvancedSourcesForEpisode(
            tvShow: TVShowContentDetail,
            episode: TVEpisode,
        ) {
            viewModelScope.launch {
        ...
        private fun mapStreamingQualityToVideoResolution(quality: SourceQuality): VideoResolution {
            return when (quality) {
                ...
                SourceQuality.QUALITY_720P_HDR,
         ...
         */
        private fun fetchAndUpdateIMDbId(
            tmdbId: Int,
            currentTvShow: TVShowContentDetail,
        ) {
