...
class SourceSorter(
    ...
    private fun calculateHealthScore(source: SourceMetadata): Int {
        ...
        // Availability bonus
        val availabilityBonus =
            source.health.availability?.let { avail ->
                (avail * 10).toInt()
            } ?: 0
...
data class SortingConfiguration(
    ...
    val poorReleaseGroups: Set<String> =
        setOf(
            "KORSUB",
            "HC",
            "HDCAM",
    ...
    val bannedReleaseGroups: Set<String> =
        setOf(
            "YIFY-SUBS",
            "FAKE",
            "VIRUS",
