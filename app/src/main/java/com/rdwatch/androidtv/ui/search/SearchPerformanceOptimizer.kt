 ...
 */
@Singleton
class SearchPerformanceOptimizer @Inject constructor() {
    ...
    fun optimizeSearch(
    ...
    ): Flow<OptimizedSearchResult> = flow {
        ...
        try {
            ...
            val results = withTimeout(30000L) { // 30 second timeout
     ...
     */
    private fun cacheSearchResults(cacheKey: String, results: List<SearchResultItem>) {
        // Implement LRU eviction if cache is full
        if (searchCache.size >= maxCacheSize) {
            val oldestKey = searchCache.entries
    ...
    fun getPerformanceStats(): PerformanceStats {
        ...
        val averageResponseTime = if (totalQueries > 0) {
        ...
        val slowQueries = queryMetrics.values
    ...
    fun getCacheStats(): CacheStats {
        val currentTime = System.currentTimeMillis()
        val expiredEntries = searchCache.values.count { 
        ...
        return CacheStats(
            ...
            averageEntryAge = if (searchCache.isNotEmpty()) {
