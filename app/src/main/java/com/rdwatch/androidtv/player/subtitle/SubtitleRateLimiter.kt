 ...
 */
@Singleton
class SubtitleRateLimiter @Inject constructor() {
     ...
     */
    suspend fun canMakeRequest(provider: SubtitleApiProvider): Boolean = mutex.withLock {
     ...
     */
    suspend fun recordRequest(provider: SubtitleApiProvider, success: Boolean = true) = mutex.withLock {
     ...
     */
    suspend fun resetAllLimits() = mutex.withLock {
        val now = System.currentTimeMillis()
    ...
    private fun initializeProviderLimits() {
        ...
        providerLimits[SubtitleApiProvider.SUBDL] = ProviderRateLimit(
            maxRequestsPerWindow = 100, // Generous limit for free API
            windowDurationMs = 60 * 60 * 1000 // 1 hour window
        )
        
        providerLimits[SubtitleApiProvider.SUBDB] = ProviderRateLimit(
            maxRequestsPerWindow = 200, // Higher limit, simple hash-based API
            windowDurationMs = 60 * 60 * 1000 // 1 hour window
        )
        
        providerLimits[SubtitleApiProvider.PODNAPISI] = ProviderRateLimit(
            maxRequestsPerWindow = 50, // Conservative limit for web scraping
            windowDurationMs = 60 * 60 * 1000 // 1 hour window
        )
        
        providerLimits[SubtitleApiProvider.ADDIC7ED_ALT] = ProviderRateLimit(
            maxRequestsPerWindow = 30, // Very conservative for TV shows
            windowDurationMs = 60 * 60 * 1000 // 1 hour window
        )
        
        providerLimits[SubtitleApiProvider.LOCAL_FILES] = ProviderRateLimit(
            maxRequestsPerWindow = Int.MAX_VALUE, // No limit for local files
            windowDurationMs = 1000 // Minimal window
        )
    }
    
    private fun resetWindow(provider: SubtitleApiProvider, now: Long) {
        val limit = providerLimits[provider] ?: return
