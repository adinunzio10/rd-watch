...
interface SubtitleParser {
     ...
     */
    suspend fun parse(content: String, encoding: String = "UTF-8"): SubtitleTrackData
    
    /**
     * Parse subtitle content from InputStream
     */
    suspend fun parse(inputStream: InputStream, encoding: String = "UTF-8"): SubtitleTrackData
...
sealed class SubtitleParseResult {
    ...
    fun getOrNull(): SubtitleTrackData? = when (this) {
        is Success -> trackData
        is Error -> null
    }
    
    fun getOrThrow(): SubtitleTrackData = when (this) {
        is Success -> trackData
...
object SubtitleParsingUtils {
    ...
    fun parseTimeString(timeString: String): Long {
        ...
        val matchResult = timeRegex.find(cleanTime)
        ...
        val milliseconds = if (matchResult.groupValues.size > 4) {
     ...
     */
    fun validateTiming(startTime: Long, endTime: Long, lineNumber: Int? = null): Boolean {
