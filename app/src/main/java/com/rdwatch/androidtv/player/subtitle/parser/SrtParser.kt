...
class SrtParser @Inject constructor() : SubtitleParser {
    ...
    private fun parseSubtitleBlock(block: String, blockNumber: Int): SubtitleCue? {
    ...
    private fun parseTimingLine(timingLine: String, blockNumber: Int): Pair<Long, Long> {
        val timingRegex = """(\d{2}:\d{2}:\d{2},\d{3})\s*-->\s*(\d{2}:\d{2}:\d{2},\d{3})""".toRegex()
        val matchResult = timingRegex.find(timingLine)
            ?: throw SubtitleParsingException(
        ...
        return try {
            when {
                ...
                colorString.startsWith("rgb(") -> {
                    ...
                    val rgbValues = colorString.removePrefix("rgb(").removeSuffix(")")
