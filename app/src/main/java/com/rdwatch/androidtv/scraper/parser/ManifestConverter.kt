 ...
 */
@Singleton
class ManifestConverter @Inject constructor() {
    ...
    fun fromEntity(entity: ScraperManifestEntity): ScraperManifest? {
        return try {
            ...
            val stremioManifest = stremioManifestJson?.let { json ->
                moshi.adapter(com.rdwatch.androidtv.scraper.models.StremioManifest::class.java).fromJson(json)
            } ?: return null
            
            val capabilities = (configMap["capabilities"] as? List<*>)?.mapNotNull { capability ->
                try {
            ...
            ScraperManifest(
                ...
                configuration = com.rdwatch.androidtv.scraper.models.ManifestConfiguration(
     ...
     */
    fun toManifestEntry(manifest: ScraperManifest, manifestId: Long = 0): ManifestEntry {
        val configJson = try {
            val configMap = mapOf(
                "providers" to manifest.configuration.providers,
                "qualityFilters" to manifest.configuration.qualityFilters,
    ...
    fun fromManifestEntry(entry: ManifestEntry): ScraperManifest? {
        return try {
            ...
            val stremioManifest = com.rdwatch.androidtv.scraper.models.StremioManifest(
    ...
    private fun getListFromConfig(config: Map<String, Any>, key: String): List<String> {
        return (config[key] as? List<*>)?.mapNotNull { it?.toString() } ?: emptyList()
    }
    
    private fun getMapFromConfig(config: Map<String, Any>, key: String): Map<String, String> {
        return (config[key] as? Map<*, *>)?.mapNotNull { (k, v) ->
            k?.toString()?.let { keyStr -> v?.toString()?.let { valueStr -> keyStr to valueStr } }
        }?.toMap() ?: emptyMap()
    }
    
    private fun getBooleanFromConfig(config: Map<String, Any>, key: String, default: Boolean): Boolean {
        return (config[key] as? Boolean) ?: default
    }
    
    private fun getLongFromConfig(config: Map<String, Any>, key: String, default: Long): Long {
