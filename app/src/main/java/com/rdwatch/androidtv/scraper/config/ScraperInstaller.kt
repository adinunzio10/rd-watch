...
class ScraperInstaller
    ...
    constructor(
                    ...
                    is ManifestResult.Success -> {
                        ...
                        val updateSummary =
                            UpdateSummary(
                                ...
                                updateResults =
                                    refreshSummary.results.map { refreshResult ->
                                        ScraperUpdateResult(
        ...
        private fun updateScraperStatus(
            scraperId: String,
            status: ScraperInstallStatus,
        ) {
        ...
        private fun getEstimatedBenefit(scraper: ScraperManifest): String {
            return when {
                scraper.metadata.capabilities.contains(
                    com.rdwatch.androidtv.scraper.models.ManifestCapability.STREAM,
                ) -> "High - Provides streaming sources"
                scraper.metadata.capabilities.contains(
                    com.rdwatch.androidtv.scraper.models.ManifestCapability.META,
                ) -> "Medium - Enhances content information"
                scraper.metadata.capabilities.contains(
                    com.rdwatch.androidtv.scraper.models.ManifestCapability.SUBTITLES,
