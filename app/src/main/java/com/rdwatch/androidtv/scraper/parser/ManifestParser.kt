...
class ManifestParser @Inject constructor() {
    ...
    fun parseManifest(
        ...
        return try {
            val detectedFormat = if (format == ManifestFormat.AUTO_DETECT) {
     ...
     */
    private fun parseJsonManifest(content: String, sourceUrl: String): ManifestResult<StremioManifest> {
        return try {
            val manifest = stremioManifestAdapter.fromJson(content)
     ...
     */
    private fun parseYamlManifest(content: String, sourceUrl: String): ManifestResult<StremioManifest> {
     ...
     */
    private fun parseXmlManifest(content: String, sourceUrl: String): ManifestResult<StremioManifest> {
     ...
     */
    private fun detectFormat(content: String, sourceUrl: String): ManifestFormat {
     ...
     */
    private fun validateBasicStructure(manifest: StremioManifest, sourceUrl: String): ManifestException? {
        return when {
            manifest.id.isBlank() -> ManifestParsingException(
                "Manifest ID cannot be empty",
                url = sourceUrl
            )
            manifest.name.isBlank() -> ManifestParsingException(
                "Manifest name cannot be empty",
                url = sourceUrl
            )
            manifest.version.isBlank() -> ManifestParsingException(
                "Manifest version cannot be empty",
                url = sourceUrl
            )
            manifest.resources.isEmpty() && manifest.catalogs.isEmpty() -> ManifestParsingException(
    ...
    fun convertToScraperManifest(
        ...
        return ScraperManifest(
            ...
            metadata = com.rdwatch.androidtv.scraper.models.ManifestMetadata(
                sourceUrl = sourceUrl,
                createdAt = now,
                updatedAt = now,
