...
object ScraperTestRunner {
    ...
    private fun testUrlGeneration(): List<String> {
        ...
        try {
            ...
            val torrentioUrl = queryBuilder.buildContentQueryUrl(
            ...
            val knightCrawlerUrl = queryBuilder.buildContentQueryUrl(
            ...
            // Test TV show format
            val tvUrl = queryBuilder.buildContentQueryUrl(
                manifest = torrentioManifest,
                contentType = "tv",
                contentId = "tt0903747",
                imdbId = "tt0903747",
    ...
    private fun testResponseParsing(): List<String> {
        ...
        try {
            ...
            val sources = responseMapper.parseScraperResponse(
                manifest = torrentioManifest,
            ...
            if (sources.size == 2) {
                ...
                if (has4K) results.add("✅ 4K source detected")
                else results.add("❌ 4K source not detected")
                
                if (has1080p) results.add("✅ 1080p source detected")
                else results.add("❌ 1080p source not detected")
                
                if (allHaveMagnetUrls) results.add("✅ All sources have magnet URLs")
                else results.add("❌ Some sources missing magnet URLs")
                
                if (allAvailable) results.add("✅ All sources marked as available")
                else results.add("❌ Some sources not available")
