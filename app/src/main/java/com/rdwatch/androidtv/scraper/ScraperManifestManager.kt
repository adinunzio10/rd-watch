 ...
 */
@Singleton
class ScraperManifestManager @Inject constructor(
    ...
    suspend fun importManifests(urls: List<String>): ManifestResult<ImportResult> {
        return try {
            ...
            urls.forEach { url ->
                val result = when (val addResult = addManifestFromUrl(url)) {
                    is ManifestResult.Success -> ManifestImportResult(
                        url = url,
                        success = true,
                        manifest = addResult.data,
                        error = null
                    )
                    is ManifestResult.Error -> ManifestImportResult(
                        url = url,
                        success = false,
                        manifest = null,
            ...
            when (val manifestsResult = repository.getManifests()) {
                is ManifestResult.Success -> {
                    ...
                    manifests.forEach { manifest ->
                        val result = when (val refreshResult = refreshManifest(manifest.id)) {
                            is ManifestResult.Success -> ManifestRefreshResult(
                                id = manifest.id,
                                success = true,
                                updatedManifest = refreshResult.data,
                                error = null
                            )
                            is ManifestResult.Error -> ManifestRefreshResult(
                                id = manifest.id,
                                success = false,
     ...
     */
    suspend fun setManifestEnabled(id: String, enabled: Boolean): ManifestResult<Unit> {
        return try {
    ...
    suspend fun getStatistics(): ManifestResult<ManagerStatistics> {
        return try {
            ...
            when (repoStats) {
                ...
                is ManifestResult.Error -> repoStats.map { 
                    ManagerStatistics(
