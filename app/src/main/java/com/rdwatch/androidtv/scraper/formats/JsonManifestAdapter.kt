 ...
 */
@Singleton
class JsonManifestAdapter @Inject constructor() {
     ...
     */
    fun parseJson(json: String, sourceUrl: String? = null): ManifestResult<StremioManifest> {
        return try {
     ...
     */
    fun toJson(manifest: StremioManifest, pretty: Boolean = true): ManifestResult<String> {
        return try {
            val adapter = if (pretty) {
     ...
     */
    private fun parseFromRawData(data: Map<String, Any>, sourceUrl: String?): ManifestResult<StremioManifest> {
        return try {
    ...
    private fun extractStringList(data: Map<String, Any>, key: String): List<String> {
        return when (val value = data[key]) {
            ...
            if (resourceItem is Map<*, *>) {
                try {
                    ...
                    StremioManifestResource(
                        ...
                        types = extractStringList(resourceMap, "types").takeIf { it.isNotEmpty() } 
    ...
    private fun extractExtras(data: Map<String, Any>): List<StremioManifestExtra>? {
        ...
        val extras = extrasData.mapNotNull { extraItem ->
            if (extraItem is Map<*, *>) {
                try {
    ...
    private fun extractBoolean(data: Map<String, Any>, key: String): Boolean? {
        return when (val value = data[key]) {
            is Boolean -> value
    ...
    private fun extractInt(data: Map<String, Any>, key: String): Int? {
        return when (val value = data[key]) {
    ...
    private fun errorResult(message: String, sourceUrl: String?): ManifestResult<StremioManifest> {
    ...
    fun validateJsonStructure(json: String): ManifestResult<Boolean> {
        return try {
            ...
            if (data == null) {
                ...
                val hasRequiredFields = data.containsKey("id") && 
