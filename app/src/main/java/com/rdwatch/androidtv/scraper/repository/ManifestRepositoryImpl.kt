...
class ManifestRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun getManifests(): ManifestResult<List<ScraperManifest>> {
            return withContext(Dispatchers.IO) {
                try {
                    ...
                    val manifests =
                        entities.first().mapNotNull { entity ->
            ...
            return withContext(Dispatchers.IO) {
                try {
                    ...
                    if (existingEntity != null) {
                        val updatedEntity =
                            converter.toEntity(manifest).copy(
        ...
        override suspend fun enableManifest(
            id: String,
            enabled: Boolean,
        ): ManifestResult<Unit> {
            return withContext(Dispatchers.IO) {
        ...
        override suspend fun updateManifestPriority(
            id: String,
            priority: Int,
        ): ManifestResult<Unit> {
            return withContext(Dispatchers.IO) {
                ...
                when (val manifestResult = getManifest(id)) {
                    is ManifestResult.Success -> {
                        ...
                        when (val fetchResult = fetchManifestFromUrl(manifest.sourceUrl)) {
                            is ManifestResult.Success -> {
                                val refreshedManifest =
                                    fetchResult.data.copy(
                                        id = manifest.id,
        ...
        override suspend fun getManifestsByCapability(capability: ManifestCapability): ManifestResult<List<ScraperManifest>> {
            return withContext(Dispatchers.IO) {
                when (val manifestsResult = getManifests()) {
                    is ManifestResult.Success -> {
                        val filteredManifests =
                            manifestsResult.data.filter { manifest ->
                ...
                when (val manifestsResult = getManifests()) {
                    is ManifestResult.Success -> {
                        val validationResults =
                            manifestsResult.data.associate { manifest ->
        ...
        override fun observeManifest(id: String): Flow<ManifestResult<ScraperManifest>> =
            flow {
                ...
                try {
                    when (val manifestsResult = getManifests()) {
                        is ManifestResult.Success -> {
                            ...
                            val byAuthor =
                                manifests.groupBy { it.author ?: "Unknown" }
