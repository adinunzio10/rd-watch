...
class TMDbApiKeyInterceptor @Inject constructor() : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        android.util.Log.d("TMDbApiKeyInterceptor", "API key starts with: ${if (BuildConfig.TMDB_API_KEY.isNotEmpty()) BuildConfig.TMDB_API_KEY.take(8) + "..." else "EMPTY"}")
        android.util.Log.d("TMDbApiKeyInterceptor", "Request already has api_key param: ${originalRequest.url.queryParameter(API_KEY_PARAM) != null}")
        ...
        if (BuildConfig.TMDB_API_KEY.isNotEmpty() && 
            ...
            android.util.Log.w("TMDbApiKeyInterceptor", "NOT adding API key - isEmpty: ${BuildConfig.TMDB_API_KEY.isEmpty()}, hasParam: ${originalRequest.url.queryParameter(API_KEY_PARAM) != null}")
        }
